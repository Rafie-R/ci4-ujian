...
<section class="bg-gradient-to-r from-accent-dark via-accent to-accent-dark text-white py-14 border-b border-primary/20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span class="text-xs uppercase font-extrabold tracking-widest text-primary-light block mb-2">Pilihan Kuliner Khas Bali</span>
        <h1 class="text-3xl sm:text-5xl font-serif font-black tracking-tight">
            Daftar Menu <span class="text-primary italic">Ashraf Betutu</span>
        </h1>
...
<section class="py-6 bg-white border-b border-gray-200 sticky top-20 z-30 shadow-sm backdrop-blur-md bg-white/95">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-none">
                ...
                <a href="<?= base_url('menu' . ($sortParam ? '?' . ltrim($sortParam, '&') : '')) ?>" 
                   class="px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all shrink-0 <?= $isAll ? 'bg-accent text-primary shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' ?>">
                    Semua Kategori
                </a>
                ...
                <?php foreach ($categories as $cat) : ?>
                    ...
                    ?>
                    <a href="<?= $catUrl ?>" 
                       class="px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all shrink-0 <?= $isActive ? 'bg-accent text-primary shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' ?>">
                        <?= esc($cat) ?>
                    </a>
                <?php endforeach; ?>
            </div>
...
<!-- Menu Listing Grid -->
<section class="py-12 bg-[#FAF9F5] min-h-[550px]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php if (! empty($menus)) : ?>
            <!-- Cards Grid (1-to-1 unique image per menu item) -->
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-xl hover:border-primary/50 transition-all duration-300 flex flex-col group">
                        ...
                        <a href="<?= base_url('menu/' . $menu['slug']) ?>" class="block h-52 bg-gradient-to-tr from-accent to-accent-light relative overflow-hidden">
                            ...
                            <div class="absolute top-3 left-3">
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-accent-dark/90 text-primary-light border border-primary/30 shadow">
                                    <?= esc($menu['category']) ?>
                                </span>
                            </div>
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between space-y-4">
                            ...
                            <div class="pt-3 border-t border-gray-100 flex items-center justify-between">
                                ...
                                <a href="<?= base_url('menu/' . $menu['slug']) ?>" 
                                   class="px-3.5 py-1.5 rounded-lg bg-primary hover:bg-primary-light text-accent-dark text-xs font-extrabold uppercase tracking-wider transition-all shadow-sm transform hover:-translate-y-0.5">
                                    Lihat Detail
                                </a>
                            </div>
                        </div>
        ...
        <?php else : ?>
            ...
            <div class="py-20 text-center max-w-md mx-auto">
                <div class="w-16 h-16 mx-auto rounded-full bg-primary/20 text-accent flex items-center justify-center text-2xl mb-4 font-serif font-bold">
                    !
                </div>
