...
<div class="bg-accent-dark text-white border-b border-primary/20 py-4">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-3xl border border-gray-200/90 shadow-xl overflow-hidden">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-0">
                ...
                <div class="lg:col-span-6 relative bg-gradient-to-tr from-accent to-accent-light min-h-[360px] lg:min-h-[520px] overflow-hidden flex items-center justify-center">
                    ...
                    <div class="absolute top-5 left-5">
                        <span class="px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider bg-accent-dark/95 text-primary-light border border-primary/40 shadow-lg">
                            <?= esc($menu['category']) ?>
                        </span>
                    </div>
                    ...
                    <div class="absolute top-5 right-5">
                        <?php if ($menu['is_available']) : ?>
                            <span class="px-3.5 py-1.5 rounded-full text-xs font-bold bg-green-900/95 text-green-300 backdrop-blur border border-green-500/40 shadow-lg">
                                ● Menu Tersedia
                            </span>
                        <?php else : ?>
                            <span class="px-3.5 py-1.5 rounded-full text-xs font-bold bg-red-900/95 text-red-300 backdrop-blur border border-red-500/40 shadow-lg">
                                ● Sedang Habis
                            </span>
                        <?php endif; ?>
                    </div>
                ...
                <div class="lg:col-span-6 p-8 sm:p-10 lg:p-12 flex flex-col justify-between space-y-8">
                    <div>
                        ...
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <span class="text-xs uppercase font-extrabold tracking-widest text-primary-dark">Kuliner Khas Bali &bull; Ashraf Betutu</span>
                        </div>

                        <!-- Menu Title -->
                        <h1 class="text-3xl sm:text-4xl font-serif font-black text-gray-900 leading-tight">
                            <?= esc($menu['name']) ?>
                        </h1>
                        ...
                        <div class="mt-4 flex items-baseline gap-2 pb-6 border-b border-gray-100">
                            ...
                            <span class="text-3xl sm:text-4xl font-serif font-black text-accent-dark">
                        ...
                        <div class="mt-6 space-y-3">
                            <h3 class="text-xs uppercase font-extrabold tracking-wider text-gray-400">Deskripsi Lengkap:</h3>
                        ...
                        <div class="mt-8 grid grid-cols-2 gap-3 pt-6 border-t border-gray-100 text-xs text-gray-600">
                            <div class="flex items-center gap-2 p-3 rounded-xl bg-offwhite border border-gray-200">
                                <span class="text-primary font-bold text-sm">✓</span>
                                <span class="font-medium text-gray-800">16 Rempah Asli Bali</span>
                            </div>
                            <div class="flex items-center gap-2 p-3 rounded-xl bg-offwhite border border-gray-200">
                                <span class="text-primary font-bold text-sm">✓</span>
                                <span class="font-medium text-gray-800">Ayam Segar Pilihan</span>
                            </div>
                            <div class="flex items-center gap-2 p-3 rounded-xl bg-offwhite border border-gray-200">
                                <span class="text-primary font-bold text-sm">✓</span>
                                <span class="font-medium text-gray-800">100% Halal & Higienis</span>
                            </div>
                            <div class="flex items-center gap-2 p-3 rounded-xl bg-offwhite border border-gray-200">
                                <span class="text-primary font-bold text-sm">✓</span>
                                <span class="font-medium text-gray-800">Aroma Daun Pisang Alami</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-4">
                            <?php if ($menu['is_available']) : ?>
                                <a href="https://wa.me/6281234567890?text=Halo%20Ashraf%20Betutu,%20saya%20ingin%20memesan%20<?= urlencode($menu['name']) ?>" 
                                   target="_blank" 
                                   class="w-full sm:flex-1 inline-flex items-center justify-center gap-2 py-4 px-6 rounded-full bg-accent hover:bg-accent-light text-white font-bold text-sm tracking-wider transition-all shadow-md hover:shadow-xl transform hover:-translate-y-0.5">
                            ...
                            <?php else : ?>
                                <button disabled class="w-full sm:flex-1 py-4 px-6 rounded-full bg-gray-200 text-gray-500 font-bold text-sm cursor-not-allowed">
                                    Menu Sedang Tidak Tersedia
                                </button>
                            <?php endif; ?>

                            <a href="<?= base_url('menu') ?>" class="w-full sm:w-auto inline-flex items-center justify-center py-4 px-8 rounded-full border border-gray-300 text-gray-700 hover:bg-gray-100 font-semibold text-sm transition-colors">
                                &larr; Semua Menu
                            </a>
                        </div>
                    </div>
        ...
        <?php if (! empty($relatedMenus)) : ?>
            <div class="mt-16">
                <div class="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
                    <div>
                        <span class="text-xs uppercase font-extrabold tracking-widest text-primary-dark">Rekomendasi Menu Lainnya</span>
                        <h2 class="text-2xl sm:text-3xl font-serif font-bold text-accent-dark mt-1">Pilihan Hidangan Betutu Favorit</h2>
                    </div>
                    <a href="<?= base_url('menu') ?>" class="text-xs font-bold text-accent hover:text-accent-light uppercase tracking-wider">
                        Lihat Semua &rarr;
                    </a>
                </div>
                    ...
                    <?php foreach ($relatedMenus as $item) : ?>
                        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col group">
                            <div class="h-44 bg-gradient-to-tr from-accent to-accent-light relative overflow-hidden">
                                <img src="<?= menu_image_url($item['image']) ?>" alt="<?= esc($item['name']) ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                <div class="absolute top-3 left-3">
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-accent-dark/90 text-primary-light border border-primary/30">
                                        <?= esc($item['category']) ?>
                                    </span>
                                </div>
                            </div>
